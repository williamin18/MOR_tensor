function CP = eval_third_order_cp(fstruct, x)
    % eval_third_order_cp  Compute the third-order derivative tensor in CP format.
    %
    %   CP = eval_third_order_cp(fstruct, x)
    %
    %   Inputs:
    ...
    %
    %   Output:
    %     CP - m x R x 4 array storing the CP decomposition of the tensor T.
    %          R is the number of nonlinear elements (diodes + BJTs).
    %          The full tensor can be recovered as:
    %            T(i,j,l,p) = sum_{r=1}^{R} CP(i,r,1)*CP(j,r,2)*CP(l,r,3)*CP(p,r,4)
    %          The scalar weight scale3 is folded into factor 1: CP(:,r,1) = scale3 * d_e.
    %
    ...
    for k = 1:length(fstruct)
        ...
        % Resistors are linear: no third-order contribution.
    end
end
