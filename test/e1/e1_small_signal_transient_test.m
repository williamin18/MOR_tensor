...
%       are computed once before the time loop.
%
% Pass criteria:
...
for t_idx = 1:n_steps
    ...
    % (B) Taylor Newton: J(x_k) ≈ J0 + sum_j H0(:,j,:) * (x_k - x_dc)(j).
    x_k       = x_dc;
    converged = false;
    for iter = 1:max_iter
        r = eval_nonlinear(f, x_k) - b_t;
